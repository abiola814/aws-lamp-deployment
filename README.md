...
![AWS Screenshot](images/AWS_PAGE1.png)

![AWS Screenshot](images/AWS_PAGE2.png)

![AWS Screenshot](images/AWS_PAGE3.png)
...
Visit `http://<ec2-public-ip>` to verify Apache is running.
...
```bash
sudo chown -R $USER:$USER /var/www/awslamp
```
...
```bash
sudo vi /etc/apache2/sites-available/awslamp.conf
```
- Copy and paste the text below
```apacheconf
<VirtualHost *:80>
    ServerName awslamp
    ServerAlias www.awslamp
    ServerAdmin webmaster@localhost
    DocumentRoot /var/www/awslamp
...
```
- The new website is active, but the web root */var/www/awslamp* is still empty. create an index.ht,l file in that location
```bash
...
http://169.254.169.254/latest/meta-data/public-ipv4) > /var/www/awslamp/index.html
```
...
```bash
echo "<?php phpinfo(); ?>" | sudo tee /var/www/awslamp/index.php
```

- Open browser and enter: `http://<ec2-public-ip>`
- Test Apache, PHP, and Database connection using a test page.

![PHP Test Page](images/PHP.png)
...
- Delete `index.php` after testing to avoid exposing server info:
  ...
  ```bash
  sudo rm /var/www/awslamp/index.php
  ```
